  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          className="relative bg-white rounded-3xl overflow-hidden w-full max-w-3xl shadow-2xl z-10 max-h-[90vh] overflow-y-auto"
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="relative aspect-square bg-gray-50">
              ...
              {images.length > 1 && (
                <>
                  <button onClick={() => setImgIdx((i) => (i - 1 + images.length) % images.length)} className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center shadow">
                    <ChevronLeft size={16} />
                  </button>
                  <button onClick={() => setImgIdx((i) => (i + 1) % images.length)} className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center shadow">
            ...
            <div className="p-8 flex flex-col">
              <p className="text-xs tracking-widest uppercase text-gray-400 font-medium mb-1">{product.subcategory}</p>
              <h2 className="text-2xl font-bold text-gray-900 mb-2" style={{ fontFamily: 'Playfair Display, serif' }}>{product.name}</h2>

              <div className="flex items-center gap-2 mb-4">
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={13} className={i < Math.floor(product.rating) ? 'fill-amber-400 text-amber-400' : 'text-gray-200'} />
                  ))}
                </div>
              ...
              {/* Quantity */}
              <div className="mb-6">
                <p className="text-xs font-semibold tracking-widest uppercase text-gray-500 mb-2">Quantity</p>
              ...
              <div className="flex gap-3 mt-auto">
                <button
                  onClick={handleAdd}
                  className="flex-1 bg-gray-900 text-white py-3.5 px-6 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 hover:bg-gray-700 transition-colors"
                >
                  <ShoppingBag size={16} />
                  Add to Bag
                </button>
