...
import { CheckCircle, Truck, CreditCard, Banknote, Package, ArrowRight, Loader2 } from 'lucide-react';
    ...
    return (
      <main className="min-h-screen bg-[#fafafa] flex items-center justify-center py-20 px-4">
        ...
        >
          ...
          <div className="bg-white rounded-2xl p-5 text-left mb-6 space-y-3" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {[
                    ...
                    { name: 'address', label: 'Street Address', placeholder: '14-F, Block 7, Gulberg III', span: true },
                    { name: 'city', label: 'City', placeholder: 'Lahore', span: false },
                    { name: 'postalCode', label: 'Postal Code', placeholder: '54000', span: false },
              ...
              <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                <div className="space-y-3">
                  {SHIPPING_METHODS.map((method) => (
                    <label
                      key={method.id}
                      className={`flex items-center justify-between p-4 border-2 rounded-xl cursor-pointer transition-all ${shippingMethod === method.id ? 'border-gray-900 bg-gray-50' : 'border-gray-100 hover:border-gray-300'}`}
                    >
                      <div className="flex items-center gap-3">
                        <input type="radio" name="shipping" value={method.id} checked={shippingMethod === method.id} onChange={() => setShippingMethod(method.id)} className="accent-gray-900" />
                        <div>
              ...
              <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                {payment === 'card' && (
                  ...
                  >
                    <input placeholder="Card Number" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-amber-400 transition-colors" />
                    <div className="grid grid-cols-2 gap-3">
                      <input placeholder="MM / YY" className="border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-amber-400 transition-colors" />
                      <input placeholder="CVV" className="border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-amber-400 transition-colors" />
                    </div>
                  </motion.div>
                )}
              </div>
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-2xl p-6 sticky top-24" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                    ...
                    return (
                      <div key={`${item._id || item.id}-${item.selectedSize}`} className="flex gap-3 items-center">
                        <div className="relative shrink-0">
                          <img src={item.image || item.mainImage} alt={item.name} className="w-14 h-16 object-cover rounded-lg" />
                ...
                <button
                  type="submit"
                  disabled={placing}
                  className="mt-6 w-full bg-gray-900 text-white py-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-amber-700 transition-colors duration-200 disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {placing ? (
                    <>
